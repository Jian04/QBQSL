...
Sprite_MiniSign.prototype.setupTwinkle = function() {
    if (this._target.constructor === Game_Player) {
...
Sprite_MiniBody.prototype.updateScroll = function(force) {
    if (this._isReady && $gameSystem.miniVisibility()) {
        ...
        const rx = Math.max(0, Math.min(this.bitmap.width - pw, sx));
        const ry = Math.max(0, Math.min(this.bitmap.height - ph, sy));
        if (force || (rx !== this._scrollData.x || ry !== this._scrollData.y)) {
            this._scrollData.x = rx;
            this._scrollData.y = ry;
            this.setFrame(rx, ry, pw, ph);
            this._fog && this._fog.setFrame(rx, ry, pw, ph);
            if (force) {
